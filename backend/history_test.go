...
func TestPostgresStore_RoundTrip(t *testing.T) {
	...
	in, _ := json.Marshal(Input{Price: 100, ProfitPerUnit: 20, DiscountPct: 5})
	saved, err := store.Save(Record{Input: in, Headline: 33.3, Label: "feasible"})
	if err != nil {
		t.Fatalf("save: %v", err)
	}
	if saved.ID == 0 {
	...
	if len(items) == 0 || items[0].ID != saved.ID || items[0].Label != "feasible" {
